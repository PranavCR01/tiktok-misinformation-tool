...
# Reuse the shared model->token limits from api_helpers
try:
...
def remove_uploaded_files():
    ...
    # Original app used 'uploaded_files'; our updated flow uses 'data' and
    # Streamlit's uploader returns files directly each run. We clear both.
    if "uploaded_files" in st.session_state:
        for file_info in st.session_state.uploaded_files:
            st.session_state.pop(file_info.get("file_name"), None)
        st.session_state.pop("uploaded_files", None)

    # Also clear cached results / page state used by 2_Analysis.py
...
def tokens_check(tokens: int, model: str):
    ...
    # If tokens exceed *all* limits, warn loudly (kept for parity with original)
    ...
    if tokens > limit:
        st.error(f"Warning: Token usage ({tokens}) exceeds the limit ({limit}) for model {model}.")
    else:
        st.success(f"Token usage ({tokens}) is within the limit ({limit}) for model {model}.")
...
def keyword(keywords: str):
    """
    Tiny helper to display keyword counts from a comma-separated string.
    (Kept for compatibility with the original UI.)
    """
